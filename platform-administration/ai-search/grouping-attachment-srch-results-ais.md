...
For details on activating and enabling Document Viewer, see [Enable Document Viewer](https://www.servicenow.com/docs/access?context=enable-document-viewer&version=yokohama&pubname=yokohama-platform-user-interface&ft:locale=en-US). To learn more about EVAM property settings for search results, see [Configure EVAM display settings for search results in AI Search applications](../task/configure-search-result-card-opts.md).
