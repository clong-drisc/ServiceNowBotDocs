...
Calling this function on a JSON node tells the proxy that the value is an [Encoded query strings](https://www.servicenow.com/docs/access?context=c_EncodedQueryStrings&version=yokohama&pubname=yokohama-platform-user-interface&ft:locale=en-US) for the specified table. The proxy parses the encoded query and encrypts the values for fields in the encoded query that must be encrypted.
